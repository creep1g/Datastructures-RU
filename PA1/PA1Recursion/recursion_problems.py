...
def less_than(a, b):  # ONLY NEEDS TO WORK FOR POSITIVE INTEGERS
    '''Recursively checks if a < b by calling it self with values a and b
    decreased by one, if a == 0 before b returns True else returns False'''
    if a == 0:
        return True
    elif b == 0 or a == b:
        return False
...
def unique(arr):
    '''Calls define an empty array and calls unique_helper with user-defined
    array and function-defined array'''
    new_arr = []
...
def unique_helper(arr_first, arr_second):
    '''Helper for unique function, takes an empty list and user defined list
    and uses linear_search to look for instances in program-defined list,
    returns a list of unique elements in user-defined list'''
    if arr_first == []:
        return arr_second
