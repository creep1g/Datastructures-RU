...
class WebHistory:
    ...
    def nav_to_website(self, web_url):
        # Create a new node for website
        self.curr_page.next = dll_node(web_url, self.curr_page, self.tail)
        self.curr_page = self.curr_page.next

    def press_back(self):
        if self.curr_page is not self.head:
            self.curr_page = self.curr_page.prev

    def press_forward(self):
        if self.curr_page.next is not self.tail:
            self.curr_page = self.curr_page.next
